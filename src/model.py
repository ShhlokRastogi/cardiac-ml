...
class UpAttention(nn.Module):
    """Upscaling then Attention Gate concatenation then double conv"""
...
class OutConv(nn.Module):
    """1x1 Convolution for multi-class logits output"""
    def __init__(self, in_channels, out_channels):
        super().__init__()
...
class AttentionUNet(nn.Module):
    """
    Attention U-Net Architecture for 2D/3D Cardiac MRI Segmentation.
    Inputs:  (B, 1, H, W) single-channel grayscale 2D MRI slice
    Outputs: (B, 4, H, W) 4-class logits (0: Background, 1: RV, 2: MYO, 3: LV)
    """
